...
import { PublicDataApiService, searchPublicFacilities } from './publicDataApiService';
...
export class FacilityService {
  ...
  static async searchFacilities(keyword: string, filters?: FacilitySearchParams): Promise<FacilityProfileResponse[]> {
    ...
    if (filters?.facilityType && filters.facilityType !== '전체') {
      mockFacilities = mockFacilities.filter(f => f.facilityType === filters.facilityType);
    }
    if (filters?.region && filters.region !== '전체') {
      mockFacilities = mockFacilities.filter(f => f.region === filters.region);
    }
    
    // 공공데이터 통합 검색 (병렬 처리)
    try {
      const publicFacilities = await PublicDataApiService.searchFacilities({
        keyword: keyword.trim() || undefined,
        region: filters?.region && filters.region !== '전체' ? filters.region : undefined,
        facilityType: filters?.facilityType && filters.facilityType !== '전체' ? filters.facilityType : undefined,
        limit: 20
      });
      
      // Mock 데이터와 공공데이터 병합 (중복 제거)
      const combinedFacilities = [...mockFacilities];
      
      publicFacilities.forEach(publicFacility => {
        // 이름과 주소가 유사한 시설은 중복으로 간주하고 제외
        const isDuplicate = combinedFacilities.some(mockFacility => 
          mockFacility.facilityName === publicFacility.facilityName ||
          mockFacility.address === publicFacility.address
        );
        
        if (!isDuplicate) {
          combinedFacilities.push(publicFacility);
        }
      });
      
      return combinedFacilities;
    } catch (error) {
      console.warn('공공데이터 검색 실패, Mock 데이터만 사용:', error);
