...
const Header: React.FC<HeaderProps> = memo(({
  ...
  // 기존 인증 상태 (레거시 호환성)
  const { isAuthenticated: isLegacyAuthenticated, user: storeUser, logout: legacyLogout } = useAuthStore();
  const navigate = useNavigate();
  const location = useLocation();
  
  // 통합 인증 상태 결정
  const isAuthenticated = isClerkEnabled ? isClerkAuthenticated : isLegacyAuthenticated;
  const logout = isClerkEnabled ? clerkLogout : legacyLogout;
  
  // 사용자 정보는 props 우선, 그 다음 Clerk, 마지막으로 store에서 가져오기
  const user = propUser || (isClerkEnabled && clerkAuthUser) || storeUser;
              ...
              <SignedIn>
                <div className="user-menu">
                  <UserButton 
                    ...
                    appearance={{
                      elements: {
                        avatarBox: "user-avatar linear-button-ghost",
                        userButtonPopoverCard: "linear-dropdown user-dropdown",
                        userButtonPopoverFooter: "dropdown-footer",
                      }
                    }}
                  />
                </div>
              ...
              <SignedOut>
                <div className="auth-buttons">
                  <SignInButton mode="modal">
                    <button className="linear-button-ghost auth-signin-btn">
                      Sign In
                    </button>
                  </SignInButton>
                  <button
