...
### **6. 🔒 보안 관리 원칙 (NEW! - 2025-07-30)**
- ✅ **환경변수 분리**: 모든 API 키, 비밀번호는 .env 파일에서만 관리
- ✅ **문서 보안 검토**: 커밋 전 모든 MD 파일에서 민감한 정보 제거 확인
- ✅ **플레이스홀더 사용**: .env.example에는 실제 값 대신 your-api-key-here 형태 사용
- ✅ **환경변수 참조**: Spring Boot application.yml에서 ${ENV_VAR} 패턴 사용
- ✅ **보안 체크리스트**: 매 커밋 전 API 키, 비밀번호, 토큰 노출 여부 점검
- ✅ **Git 히스토리 정리**: 민감한 정보 노출 시 즉시 수정 커밋 및 필요시 히스토리 정리
...
- ❌ **문서나 소스코드에 API 키 하드코딩** (반드시 환경변수 참조 사용)
- ❌ **Git 커밋에 민감한 정보 포함** (커밋 전 보안 검토 필수)
...
- ✅ 실제 운영을 위한 완전한 코드
- ✅ **API 키 보안 원칙**: 모든 민감한 정보는 .env 파일에서만 관리, 문서나 소스코드에 하드코딩 금지
